document.addEventListener("DOMContentLoaded", function() {
	...
	document.querySelectorAll('.flickr-gallery-block').forEach((block) => {
		block.querySelectorAll('.flickr-gallery-block__pagination').forEach((pagination) => {
			const totalPages = Number(pagination.dataset.pages)
			updatePagination(1, false)

			function changePage (event) {
				updatePagination(Number(event.currentTarget.dataset.page))
			}

			function updatePagination (currentPage, fetchPage = true) {
