...
function settings_register_fields()
{
    ...
    add_settings_field('contact', '<label for="contact">' . __('Endereço', 'jaci') . '</label>', 'jaci\\settings_contact_html', 'general');

    register_setting('reading', 'excerpt_size', 'esc_attr');
    add_settings_field('excerpt_size', '<label for="excerpt_size">' . __('Número de palavras do resumo automático (excerpt)', 'jaci') . '</label>', 'jaci\\settings_excerpt_size_html', 'reading');
}
