...
function flickr_get_contents( $api_key, $flickr_by_type = 'user', $data_id = '' ) {
    ...
    if ( $flickr_by_type == 'user' ) {
        ...
        $request_api = 'https://api.flickr.com/services/rest/?method=flickr.photosets.getPhotos&api_key=' . $api_key . '&photoset_id=' . $data_id . '&extras=url_m,url_o&format=json&nojsoncallback=1';
    }

    if ( isset( $request_api ) ) {
        $file_get_contents = json_decode( file_get_contents( $request_api ) );

        if ( $flickr_by_type == 'user' && isset( $file_get_contents->photos->photo ) ) {
            $data = $file_get_contents->photos->photo;
        } elseif ( $data_id && isset( $file_get_contents->photoset->photo ) ) {
